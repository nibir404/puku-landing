        ...
        <Container>
          <div className="mx-auto max-w-3xl text-center">
            ...
            <h1 className="mt-5 font-display text-display-xl font-medium tracking-tight">
              Get Puku on your <span className="text-gradient">machine.</span>
            </h1>
          ...
          <div className="mx-auto mt-16 grid max-w-5xl grid-cols-1 gap-4 md:grid-cols-3">
            {platforms.map((p) => (
              <a
                key={p.name}
                href={p.href}
                className="group block rounded-2xl border border-border bg-surface p-7 transition-all duration-200 hover:border-accent/40 hover:shadow-card"
              >
                <div className="flex h-12 w-12 items-center justify-center rounded-xl bg-[#f4f4f5] text-ink">
                  {p.icon}
                </div>
                <h3 className="mt-5 font-mono text-base font-semibold uppercase tracking-wider text-ink">{p.name}</h3>
                <div className="mt-2 text-[12.5px] text-ink-muted">{p.arch}</div>
                <div className="mt-4 rounded-lg border border-border bg-[#f4f4f5] p-3 font-mono text-[12px] text-ink-muted">
                  {p.file}
                </div>
                <div className="mt-3 flex items-center justify-between text-[12px] text-ink-muted">
                  <span>{p.size}</span>
                  <span className="text-accent transition-all group-hover:translate-x-1">Download →</span>
                </div>
              </a>
            ))}
          </div>

          <div className="mx-auto mt-16 max-w-3xl rounded-2xl border border-border bg-surface p-8">
            <h2 className="font-mono text-base font-semibold uppercase tracking-wider text-ink">Or, install with your package manager.</h2>
            <pre className="mt-4 overflow-x-auto rounded-lg border border-border bg-[#f4f4f5] p-4 font-mono text-[13px] text-ink">
{`# macOS
brew install puku
