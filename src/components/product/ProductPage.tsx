        ...
        <Container className="relative">
          ...
          >
            <div className="lg:col-span-7">
              ...
              <motion.div
                variants={staggerChild}
                className="mt-10 flex flex-col sm:flex-row items-center gap-3.5 w-full max-w-md"
              >
...
const HowItWorks = ({ steps }: { steps: { step: string; title: string; body: string }[] }) => (
  <SectionShell>
    <div className="mb-12 max-w-3xl">
      ...
      <h2 className="mt-5 font-display text-display-md md:text-display-lg font-bold text-[#0F0F11] tracking-tight">
    ...
    <div className="grid grid-cols-1 gap-4 md:grid-cols-2">
      {steps.map((s) => (
        <motion.div
          ...
          className="rounded-2xl border border-[#E5E5E8] bg-white p-6 hover:border-[#6E56CF] transition-colors"
        >
          <div className="text-[11px] font-bold uppercase tracking-wider text-[#6E56CF]">{s.step}</div>
          <div className="mt-1 font-mono text-lg font-bold uppercase tracking-wide text-[#0F0F11]">{s.title}</div>
          <p className="mt-3 text-[14.5px] leading-relaxed text-[#333338]">{s.body}</p>
        </motion.div>
      ))}
    </div>
  </SectionShell>
...
const WorkflowSection = ({ items }: { items: string[] }) => (
  <SectionShell className="border-y border-[#E5E5E8]">
    <div className="mb-12 max-w-3xl">
    ...
    <ol className="grid grid-cols-1 gap-4 md:grid-cols-2">
      {items.map((t, i) => (
        <li key={i} className="flex items-start gap-4 rounded-2xl border border-[#E5E5E8] bg-white p-5 hover:border-[#6E56CF] transition-colors">
          <span className="inline-flex h-7 w-7 shrink-0 items-center justify-center rounded-full border border-[#E5E5E8] bg-[#FAFAFC] text-[12px] font-bold text-[#6E56CF]">
            {i + 1}
          </span>
          <span className="text-[14.5px] leading-relaxed text-[#333338]">{t}</span>
        </li>
      ))}
    </ol>
  </SectionShell>
...
const PerformanceSection = ({ items }: { items: { value: string; label: string; hint?: string }[] }) => (
  <SectionShell>
    ...
    <div className="grid grid-cols-2 gap-4 md:grid-cols-4">
      {items.map((s) => (
        <div key={s.label} className="rounded-2xl border border-[#E5E5E8] bg-white p-6 hover:border-[#6E56CF] transition-colors">
          <div className="font-mono text-3xl font-extrabold tracking-tight text-[#6E56CF]">{s.value}</div>
          <div className="mt-1 text-[13.5px] text-[#0F0F11] font-semibold">{s.label}</div>
          {s.hint && <div className="mt-2 text-[12px] text-[#4A4A52] font-medium">{s.hint}</div>}
        </div>
      ))}
    </div>
  </SectionShell>
...
const IntegrationsSection = ({ items }: { items: { name: string; category: string }[] }) => (
  <SectionShell className="border-y border-[#E5E5E8]">
    ...
    <div className="grid grid-cols-2 gap-4 sm:grid-cols-3 md:grid-cols-4">
      {items.map((i) => (
        <div key={i.name} className="rounded-2xl border border-[#E5E5E8] bg-white p-5 hover:border-[#6E56CF] transition-colors">
          <div className="font-mono text-sm uppercase tracking-wide font-bold text-[#0F0F11]">{i.name}</div>
          <div className="text-[12.5px] text-[#4A4A52] font-medium mt-1">{i.category}</div>
        </div>
      ))}
    </div>
  </SectionShell>
    ...
    <div className="grid grid-cols-1 gap-4 md:grid-cols-3">
      {links.map((l) => (
        <Link
          key={l.name}
          to={l.href}
          className="group rounded-2xl border border-[#E5E5E8] bg-white p-6 transition-all duration-200 hover:border-[#6E56CF]"
        >
          <div className="flex items-center justify-between">
            <div className="font-mono text-sm font-bold uppercase tracking-wider text-[#0F0F11]">{l.name}</div>
            <ArrowUpRight className="h-4 w-4 text-[#6E56CF] transition-all group-hover:translate-x-0.5 group-hover:-translate-y-0.5" />
          </div>
          <p className="mt-2 text-[13.5px] text-[#333338] leading-relaxed">{l.desc}</p>
        </Link>
      ))}
    </div>
  </SectionShell>
...
const FAQSection = ({ items }: { items: { q: string; a: string }[] }) => {
  ...
  return (
    <SectionShell>
      ...
      <div className="mx-auto max-w-3xl rounded-2xl border border-[#E5E5E8] bg-white">
        {items.map((f, i) => (
          <div
            key={f.q}
            className={cn(
          ...
          >
            <button
              ...
              className="flex w-full items-center justify-between gap-6 px-6 py-5 md:px-8 hover:bg-[#FAFAFC] transition-colors text-left font-medium min-h-[44px]"
            >
              <div className="font-mono text-sm uppercase tracking-wider font-bold text-[#0F0F11]">
                {f.q}
              </div>
            ...
            >
              <div className="px-6 pb-6 md:px-8 md:pb-7 text-[14.5px] leading-relaxed text-[#333338] bg-[#FAFAFC]/50">
                {f.a}
              </div>
            </motion.div>
          </div>
